            ...
            dispatcher.register(literal("freezecameramod")
                    .then(literal("listCameras")
                            .executes(context -> {
                                ...
                                if (cameras.isEmpty()) {
                                    context.getSource().sendFeedback(Text.translatable("message.freezecamera.listCamerasNull"));
                                } else {
                                    context.getSource().sendFeedback(Text.translatable("message.freezecamera.listCamerasSuccess"));
                                    for (Map.Entry<String, SavedCameraManager.CameraMemory> entry : cameras.entrySet()) {
                                        ...
                                        context.getSource().sendFeedback(Text.translatable("message.freezecamera.listCamerasObtained", name, data.yaw(), data.pitch()));
                                    }
                                }
