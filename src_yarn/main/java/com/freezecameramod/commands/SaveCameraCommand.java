                            ...
                            .then(literal("saveCamera")
                                    .then(argument("cameraName", StringArgumentType.word())
                                            .executes(context -> {
                                                ...
                                                Text yawText = varDefault(String.valueOf(yaw));
                                                Text pitchText = varDefault(String.valueOf(pitch));

                                                SavedCameraManager.saveCamera(cameraName, yaw, pitch);

                                                context.getSource().sendFeedback(
                                                        Text.translatable("message.freezecamera.saveCameraSuccess", camNameText, yawText, pitchText)
                                                );
                                                return 1;
                                            })
                                    )
